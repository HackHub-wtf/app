...
export function NotificationCenter({ isOpen, onClose }: NotificationCenterProps) {
  ...
  const loadNotifications = async () => {
    try {
      type PageResult = { content: Array<{ id: string; title: string; message: string; type: string; read: boolean; createdAt: string }> }
      const result = await api.get<PageResult>('/api/v1/notifications?size=20&sort=createdAt,desc')
      const items = (result?.content ?? []).map(n => ({
        id: n.id,
        title: n.title,
        message: n.message,
      ...
      // Non-fatal — notifications panel stays empty on error
      setNotifications([])
    }
  }
  ...
  const markAsRead = async (notificationId: string) => {
    ...
    await api.patch(`/api/v1/notifications/${notificationId}/read`, {}).catch(() => {})
  }
