...
import type { ChatEvent } from '../contexts/RealtimeContext'
...
export function TeamChat({ teamId, teamName, isOpen = true }: TeamChatProps) {
  ...
  useEffect(() => {
    ...
    if (realtime.isConnected && teamId) {
      ...
      const unsubscribe = realtime.subscribeToTeamChat(teamId, (event: ChatEvent) => {
        const message: ChatMessage = {
          id: event.id,
          content: event.content,
          senderId: event.userId,
          senderName: event.userId.substring(0, 8),
          timestamp: new Date(event.createdAt),
          type: event.fileUrl ? 'file' : 'text',
  ...
  const sendMessage = () => {
    ...
    // Send via STOMP
    realtime.sendTeamMessage(teamId, newMessage)
  ...
  const handleFileUpload = (file: File | null) => {
    ...
    setTimeout(() => {
      ...
      realtime.sendTeamMessage(teamId, `[file] ${file.name}`)
