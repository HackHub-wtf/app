...
export function TeamChatNew({ teamId }: { teamId: string }) {
  ...
  const { isConnected, subscribeToTeamChat } = useRealtime()
  ...
  useEffect(() => {
    ...
    if (teamId) {
      ...
      const unsubscribe = subscribeToTeamChat(teamId, (_event: ChatEvent) => {
        // Reload messages on any new chat event
        loadChatHistory()
      })
  ...
  const { sendTeamMessage } = useRealtime()
  ...
  const handleSendMessage = async () => {
    ...
    try {
      setIsSending(true)
      sendTeamMessage(teamId, newMessage.trim())
      setNewMessage('')
    } catch (error) {
