...
export function FlexibleVotingInterface({
  ...
  const loadVotingData = useCallback(async () => {
    try {
      ...
      const criteriaData = await VotingService.getCriteria(hackathonId)
  ...
  const handleSubmitVote = async () => {
    ...
    try {
      ...
      for (const criteriaId of Object.keys(userScores)) {
        await VotingService.scoreIdea(ideaId, criteriaId, userScores[criteriaId])
      }
  ...
  const handleRemoveVote = () => {
    ...
    notifications.show({
      title: 'Vote Cleared',
      message: 'Your vote has been cleared locally',
      color: 'blue',
    })
    onVoteSubmitted?.()
  }
